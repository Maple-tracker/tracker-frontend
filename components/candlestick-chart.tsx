...
export function CandlestickChart({ data, width = 800, height = 400, onHover }: CandlestickChartProps) {
  ...
  useEffect(() => {
    ...
    if (!canvas || !data || data.length === 0 || containerSize.width === 0) return
    ...
    data.forEach((d, i) => {
      ...
      if (i === hoveredIndex) {
        ctx.strokeStyle = "#ffffff"
        ctx.lineWidth = 1
        ctx.strokeRect(candleX - 1, highY - 1, candleWidth + 2, lowY - highY + 2)
      }
    })
        ...
        >
          ...
          >
            <span style={{ fontSize: "0.875rem", fontWeight: 500 }}>
              {new Date(data[hoveredIndex].date).toLocaleDateString("ko-KR", { month: "short", day: "numeric" })}
            </span>
          </div>
          <div style={{ display: "grid", gap: "0.25rem", marginBottom: "0.5rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#c1abff", fontSize: "0.75rem" }}>평균가:</span>
              <span style={{ fontSize: "0.75rem", fontWeight: 500 }}>
                {data[hoveredIndex].price.toLocaleString()} 메소
              </span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#c1abff", fontSize: "0.75rem" }}>최고가:</span>
              <span style={{ color: "#4ade80", fontSize: "0.75rem", fontWeight: 500 }}>
                {data[hoveredIndex].highPrice.toLocaleString()} 메소
              </span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#c1abff", fontSize: "0.75rem" }}>최저가:</span>
              <span style={{ color: "#f87171", fontSize: "0.75rem", fontWeight: 500 }}>
                {data[hoveredIndex].lowPrice.toLocaleString()} 메소
              </span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "#c1abff", fontSize: "0.75rem" }}>거래량:</span>
              <span style={{ fontSize: "0.75rem", fontWeight: 500 }}>{data[hoveredIndex].volume} 개</span>
            </div>
          </div>
          {hoveredIndex > 0 && (
            <div
              style={{
                ...
                fontWeight: 500,
                textAlign: "center",
                color: data[hoveredIndex].price - data[hoveredIndex - 1].price >= 0 ? "#4ade80" : "#f87171",
              }}
            >
              {`전일대비: ${data[hoveredIndex].price - data[hoveredIndex - 1].price >= 0 ? "+" : ""}${(
                data[hoveredIndex].price - data[hoveredIndex - 1].price
              ).toLocaleString()} (${data[hoveredIndex].price - data[hoveredIndex - 1].price >= 0 ? "+" : ""}${(
                ((data[hoveredIndex].price - data[hoveredIndex - 1].price) / data[hoveredIndex - 1].price) * 100
              ).toFixed(2)}%)`}
            </div>
          )}
        </div>
